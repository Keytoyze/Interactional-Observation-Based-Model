...
class DNN(BaseRankingModel):
    ...
    def __init__(self, hparams_str):
        ...
        self.hparams = ultra.utils.hparams.HParams(
            ...
            norm="None",                                # Set the default normalization
            dropout=0.1
        )
    ...
    def build(self, input_list, noisy_params=None,
        ...
        print("ranking model input list: %d, %s" % (len(input_list), input_list[0].shape))
            ...
            for j in range(len(output_sizes)):
                ...
                if j < len(output_sizes) - 1:
                    if context_embedding is None:
                        context_embedding = output_data
                    else:
                        context_embedding = tf.concat([context_embedding, output_data], axis=-1)
                    prefix = ""
                else:
                    prefix = ""
                with tf.variable_scope(prefix):
                    expand_W = self.get_variable(
                        prefix + "dnn_W_%d" % j, [current_size, output_sizes[j]], noisy_params=noisy_params, noise_rate=noise_rate)
                    expand_b = self.get_variable(prefix + "dnn_b_%d" % j, [
                ...
                if j != len(output_sizes) - 1:
                    ...
                    # dropout
                    if self.hparams.dropout > 0 and j != 0:
                        output_data = tf.layers.dropout(output_data, rate=self.hparams.dropout,
                                                        training=is_training)
                # else:
                #     output_data = tf.sigmoid(output_data)
                current_size = output_sizes[j]

            return tf.split(input_data, len(input_list), axis=0), tf.split(output_data, len(input_list), axis=0)
