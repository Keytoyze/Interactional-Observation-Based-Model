"""The basic class that contains all the API needed for the implementation of a propensity model.
...
class BasePropensityModel(ABC):
    ...
    NORM_FUNC_DIC = {
        ...
        NormalizationFunctions.LAYER: tf.keras.layers.BatchNormalization
    }
    ...
    def to_prob(self, logits):
        # return tf.sigmoid(logits - tf.reduce_mean(logits, -1, keep_dims=True))
        # return tf.nn.elu(logits) + 1 + 1e-7
        # return tf.where(logits > 0, logits + 0.5, tf.sigmoid(logits))
        # return logits
        return tf.sigmoid(logits) # 1: ssh
        # return (tf.tanh(logits) + 1) / 2

        
        # return tf.math.softplus(logits) # 3