...
class RegressionEM(BaseAlgorithm):
    ...
    * Cappé, Olivier, and Eric Moulines. "Online expectation_maximization algorithm for latent data models." Journal of the Royal Statistical Society: Series B (Statistical Methodology) 71.3 (2009): 593-613.
    ...
    def __init__(self, data_set, exp_settings, forward_only=False):
        ...
        self.hparams = ultra.utils.hparams.HParams(
            ...
            propensity_learning_rate=0.05,                 # Learning rate.
        ...
        self.propensity_learning_rate = tf.Variable(self.hparams.propensity_learning_rate, trainable=False)
        ...
        reshaped_labels = tf.transpose(tf.convert_to_tensor(self.relevances))
        ...
        if not forward_only:
            ...
            self.context_embedding, train_output = self.ranking_model(
            ...
            self.raw_propensity = self.get_propensity_scores(True, reshaped_train_labels)
            ones = tf.cast(tf.ones_like(self.raw_propensity) * 0.5, tf.float32)
            w = tf.cast(self.global_step / self.hparams.soft_step / 10, tf.float32)
            self.propensity = tf.cond(self.global_step <= self.hparams.soft_step,
                       lambda: w * self.raw_propensity + (1 - w) * ones,
                       lambda: self.raw_propensity)

            self.splitted_propensity = tf.split(
                self.raw_propensity, self.rank_list_size, axis=1)
            ...
            gamma = tf.stop_gradient(tf.sigmoid(train_output))
            p_e1_r1_c1 = 1
            p_e1_r0_c0 = self.propensity * \
                (1 - gamma) / (1 - self.propensity * gamma + 1e-7)
            p_e0_r1_c0 = (1 - self.propensity) * gamma / \
                (1 - self.propensity * gamma + 1e-7)
            ...
            target = reshaped_train_labels + (1 - reshaped_train_labels) * p_e1_r0_c0
            # target = tf.ones_like(target)
            for i in range(self.rank_list_size):
                tf.summary.scalar(
                    'Target_%d' %
                    i,
                    tf.reduce_mean(target[:, i]),
                    collections=['train', 'eval'])
            self.exam_loss = tf.losses.log_loss(target, self.raw_propensity)
            self.denoise_params = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "(ranking_model/)?propensity_model")
            ...
            self.opt_denoise = tf.train.GradientDescentOptimizer(self.propensity_learning_rate)
            self.update_propensity_op = self.opt_denoise.apply_gradients(zip(denoise_gradients, self.denoise_params))
            ...
            self.ranker_labels = get_bernoulli_sample(tf.stop_gradient(p_r1))
            ...
            if self.hparams.max_gradient_norm > 0:
                ...
                self.updates = opt.apply_gradients(zip(self.gradients, params),
                                                   global_step=self.global_step)
            # self.updates = tf.group(denoise_updates, self.updates)
    ...
    def step(self, session, input_feed, forward_only):
        ...
        if not forward_only:
            ...
            # train propensity multi times
            # for i in range(10):
            #     session.run([self.update_propensity_op], input_feed)
